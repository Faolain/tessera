#!/usr/bin/env bash
# publish_s1_to_s3.sh — push S1 logs/metrics (and optionally artifacts) to S3
#
# Usage:
#   bash tessera_preprocessing/tools/publish_s1_to_s3.sh \
#     --local-root /data/s1_out \
#     --partition-id 31TCH_2024 \
#     --s3 s3://your-bucket/tessera/s1 \
#     [--upload-outputs 0|1] [--delete-local 0|1]
#
# Layout on S3:
#   s3://<bucket>/<prefix>/<partition_id>/<run_id>/
#     logs/     → s1_<partition_id>_detail.log, dask-report-*.html
#     metrics/  → s1_metrics.jsonl, summary.txt (optional)
...
LOG_FILE="${LOCAL_ROOT}/s1_${PARTITION_ID}_detail.log"
DASK_REPORT="$(ls -1 ${LOCAL_ROOT}/dask-report-${PARTITION_ID}.html 2>/dev/null || true)"
METRICS="${LOCAL_ROOT}/metrics/s1_metrics.jsonl"
...
if [[ -f "tessera_preprocessing/tools/summarize_s1_metrics.py" && -f "${METRICS}" ]]; then
  python tessera_preprocessing/tools/summarize_s1_metrics.py --metrics "${METRICS}" > "${EXPORT_DIR}/summary.txt" || true
fi
...
if [[ "${UPLOAD_OUTPUTS}" == "1" ]]; then
  aws s3 sync "${LOCAL_ROOT}/" "${BASE_S3}/artifacts/" \
    ...
    --exclude "dask-report-*.html" --exclude "s1_*_detail.log" \
